...
public class OrganizadorMainActivity extends AppCompatActivity {
    ...
    OrgaEventosFragment orgaeventosFragment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (orgaeventosFragment == null) {
            entradasFragment = new ClienteEntradasFragment();
            orgaeventosFragment = new OrgaEventosFragment();
            perfilFragment = new ClientePerfilFragment();
        }
        switch (navValue) {
            case 0:
                replaceFragment(orgaeventosFragment);
                break;
            case 1:
        ...
        binding.bottomNav.setOnItemSelectedListener(item -> {
            ...
            if (id == R.id.orgmiseventos) {
                replaceFragment(orgaeventosFragment);
                navValue = 0;
            } else if (id == R.id.orgsolicitudes) {
                replaceFragment(entradasFragment);
                navValue = 1;
            } else if (id == R.id.orgperfil) {
                replaceFragment(perfilFragment);
                navValue = 2;
            }
            return true;
